...
where 
        ...
        and e.name  not like '%relay%'
        and e.name not ilike '%synchronized%' 
        and e.name not ilike '%four%'
        and e.name not ilike '%%'
        and e.name not in ('basketball', 'baseball', 'football', 'handball', 'hockey', 'team competition', 'volleyball', 'water polo')
...
"""
medal_ids ="""select m.id from medals as m
        join events as e on m.event_id = e.id
        where e.name  not ilike ''%%team%%'' 
        and e.name  not ilike ''%%double%%'' 
        and e.name not ilike ''%%group%%'' 
        and e.name not ilike ''%%duet%%''
        and e.name  not ilike ''%%relay%%''
        and e.name not ilike ''%%synchronized%%'' 
        and e.name not ilike ''%%four%%'' 
        and e.name not ilike ''%%eight%%'' 
        and e.name not ilike ''%%quadruple%%''
        and e.name not ilike ''%%two%%''
        and e.name not ilike ''%%tandem%%''
        and e.name not ilike ''%%pair%%''
        and e.name not in (''basketball'', ''baseball'', ''football'', ''handball'', ''hockey'', ''softball'', ''team competition'', ''volleyball'', ''water polo'', ''flying dutchman'')"""
...
def create_sub_sql(created_table_id, pull_table):
  return """select {1}.{0}id from medals as m
          ...
          group by {1}.{0}id""".format(created_table_id, pull_table, medal_ids)
def main():
  """
    SPORTS
  """
